from _settings import AURIN_PROCESSED
import csv
import json
import pandas as pd

def aurin_raw_to_stats(filename):
    aurinFile = open(filename, "r")
    ...
    for index in aurinHead:
        ...
        jsontext["docs"].append({"city": cityName, "agelessthan25": ageless25, "totalnum": total_num})

    jsondata = json.dumps(jsontext, indent=4, separators=(',', ': '))

    f = open(AURIN_PROCESSED, 'w')
    f.write(jsondata)
    f.close()
